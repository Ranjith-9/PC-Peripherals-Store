...
export async function POST(req: NextRequest) {
  ...
  const { razor_payment_id, razorpay_order_id, razorpay_signature, data } =
    await req.json();

  const generatedSignature = crypto
    .createHmac("sha56", process.env.RAZORPAY_KEY_SECRET!)
    .update(razor_payment_id + "|" + razorpay_order_id)
    .digest("hex");
