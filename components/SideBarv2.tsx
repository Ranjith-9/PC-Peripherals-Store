...
export default function SideBar({ categoryFilters }: SideBarProps) {
  ...
  const isSelected = (filterName: string, value: string) => {
    return filters.subcategory[filterName]?.includes(value) ?? false;
  };
  ...
  const toggleFilter = (filterName: string, value: string) => {
    setFilters((prev) => {
      const currentValues = prev.subcategory[filterName] ?? [];
  ...
  const clearFilter = (filterName: string) => {
    setFilters((prev) => {
      ...
      delete updatedSubcategory[filterName];
  ...
  const selectedCount = (filterName: string) => {
    return filters.subcategory[filterName]?.length ?? 0;
  };
